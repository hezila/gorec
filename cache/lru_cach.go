...
// The implementation borrows from SmallLRUCache (originally by Nathan 
// Schrenk). The object maintains a doublly-linked list of elements in
// When an element is accessed it is promoted to the head of the list,
// and when space is needed the element at the tail of the list (the
// Least recently used element) is evicted.
// The source code is copied from vites (by youtube)
...
type LRUCache struct {
	...
	// Our current size, in bytes. Obviously a gross simplication 
	// and Low-grade approximation.
	size int64
